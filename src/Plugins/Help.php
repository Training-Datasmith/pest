...
use Php_Unit\Text_Ui\Help as PHPUnitHelp;
use Symfony\Component\Console\Output\Output_Interface;
/**
 * @internal
 */
final readonly class Help implements Handles_Arguments
{
    ...
    {
        if ($this->has_argument('--help', $arguments)) {
            View::render('version', ['version' => version()]);
            View::render('usage');
            foreach ($this->get_content() as $title => $options) {
                if ($title === 'Usage') {
                    continue;
                }
                $this->output->writeln(['', sprintf('  <fg=yellow;options=bold>%s OPTIONS:</>', mb_strtoupper($title))]);
                foreach ($options as $option) {
                    if (!array_key_exists('arg', $option)) {
                        continue;
                    }
                    ['arg' => $argument, 'desc' => $description] = $option;
                    assert(is_string($argument));
                    if (trim($argument) === '--process-isolation') {
                        continue;
                    }
                    View::render('components.two-column-detail', ['left' => $this->colorize_options($argument), 'right' => preg_replace(['/</', '/>/'], ['[', ']'], $description)]);
                }
            }
     ...
     */
    private function colorize_options(string $argument): string
    {
